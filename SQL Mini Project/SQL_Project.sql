...
SELECT DISTINCT Facilities.name AS  "Tennis Court", CONCAT( Members.firstname,  ' ', Members.surname ) AS name
FROM Bookings
...
JOIN Facilities ON Bookings.facid = Facilities.facid
JOIN Members ON Bookings.memid = Members.memid
AND Bookings.starttime LIKE  '2012-09-14%') as query
WHERE query.cost >30
ORDER BY query.cost DESC
